...
public sealed class TaskFeatureTests
{
    ...
    public async Task MorningReport_GivenMixedOneShotAndRecurringDueStates_WhenRequested_ThenOrdersChronologically()
    {
        ...
        var instanceStore = new MemoryRecurringTaskInstanceReader(
    ...
    private static Func<ValueTask<TaskItem>> HandlerFor(OneShotTaskStatus target, MemoryStore store) =>
        target switch
        {
    ...
    private sealed class MemoryRecurringTaskInstanceReader(params RecurringTaskInstance[] instances)
