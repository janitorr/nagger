...
7. **Commit the archive**
   ...
   After the summary is displayed, commit automatically - do not ask for
   confirmation. Stage the archived change (now under the archive directory) and
   any main specs updated by the sync - never `git add -A` or `git add .`.

   Commit with this repository's archive convention:
   ```bash
   git commit -m "archive: archive <name> change"
   ```
   Let hooks run; never use `--no-verify`. Do not push.
...
**Committed:** <short SHA and message>
