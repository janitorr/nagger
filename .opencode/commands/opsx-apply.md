...
8. **Commit the work**
   ...
   When the run completes, commit automatically - do not ask for confirmation.
   Use your own judgement for granularity and messages so the history reads
   well, following this repository's existing Conventional Commit conventions
   and the `git-workflow` skill (scopes such as `core:`, `host:`, `docs:`; see
   `git log`).

   Before committing:
   - Run `dotnet csharpier format .` (required by AGENTS.md).
   - Run the build and the tests the change touches; do not commit if either
     fails.
   - Never use `--no-verify`; let the pre-commit hook and analyzer gates run.

   Stage only the files this run changed - implementation, the change's tracked
   task file, and any related specs - never `git add -A` or `git add .`.
   Do not push.

   If the run paused or was blocked before completing, do not commit; leave the
...
**Committed:** <short SHA and message(s)>
