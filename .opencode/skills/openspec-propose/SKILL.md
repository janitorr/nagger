...
4. **Create a working branch before writing any artifacts**
...
9. **Commit the planning artifacts**

   Once the final status is reported, commit the change's artifacts
   automatically - do not ask for confirmation - so the plan is captured before
   implementation starts.

   - Stage only the change's artifacts from the change root (`changeRoot` in the
     status JSON; `openspec/changes/<name>/` for a local root), not unrelated
     working-tree changes; never use `git add -A` or `git add .`:
     ```bash
     git add "<change-root>"
     ```
   - Commit with a conventional message naming the change:
     ```bash
     git commit -m "docs: add <name> change"
     ```
   - Let hooks run; never bypass them with `--no-verify`. If the commit fails,
     report the failure and leave the changes staged.
   - Do not push; the proposal stays local until implemented.
...
- The commit created in step 9 (branch, short SHA, and message)
- Prompt: "The artifacts are ready for review and committed. When you are ready, run `/opsx-apply` or ask me to apply this change."
