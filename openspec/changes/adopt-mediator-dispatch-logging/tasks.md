...
- [x] 3.1 In `src/Nagger.Host/AppLog.cs`, remove `RequestCompleted`, `ValidationRejected`, and `UnexpectedFailure`; add source-generated log messages for dispatch success (Info) and failure (Warning for validation, Error otherwise). Keep `TaskCreated`. Verify `dotnet build Nagger.slnx`.
