...
When creation succeeds, the service SHALL persist a template and create the first one-shot task instance with a due date equal to the start date. It SHALL return `201 Created` with the template representation.
...
- **THEN** the service persists a recurring template and creates a one-shot task instance due on 2026-08-06
