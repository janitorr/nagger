...
public sealed record MorningReportItem(long Id, string Title, DateTimeOffset DueAt, string Type, string DueState, int? DaysOverdue, int? DaysUntilDue, string ReminderPolicy);
    ...
    : IQueryHandler<MorningReportQuery, MorningReport>
{
    public async ValueTask<MorningReport> Handle(MorningReportQuery query, CancellationToken cancellationToken)
    {
        ...
        foreach (var task in tasks)
            AddItem(items, ref dueToday, ref overdue, ref upcoming, reportDate, clock.TimeZone, task.Id, task.Title, task.DueAt, task.ReminderPolicy.ToContractValue(), "one-shot");
        foreach (var instance in instances)
            AddItem(items, ref dueToday, ref overdue, ref upcoming, reportDate, clock.TimeZone, instance.RecurringTaskId, instance.Title, instance.DueAt, instance.ReminderPolicy.ToContractValue(), "recurring");

        return new MorningReport("3", clock.UtcNow, reportDate, new MorningReportSummary(dueToday, overdue, upcoming), items);
    }

    private static void AddItem(List<MorningReportItem> items, ref int dueToday, ref int overdue, ref int upcoming, DateOnly reportDate, TimeZoneInfo timeZone, long id, string title, DateTimeOffset dueAt, string reminderPolicy, string type)
    {
        ...
        {
            overdue++;
            items.Add(new MorningReportItem(id, title, dueAt, type, "overdue", reportDate.DayNumber - itemDate.DayNumber, null, reminderPolicy));
        }
        else if (itemDate <= reportDate.AddDays(7))
        {
            upcoming++;
            items.Add(new MorningReportItem(id, title, dueAt, type, "upcoming", null, itemDate.DayNumber - reportDate.DayNumber, reminderPolicy));
        }
    }
}
