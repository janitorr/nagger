...
public sealed class SqliteTaskStore(NaggerDbContext database) : ITaskStore
{
    ...
    {
        var entity = new TaskEntity { Title = task.Title, DueAt = task.DueAt, ReminderPolicy = task.ReminderPolicy.ToContractValue(), CreatedAt = task.CreatedAt, UpdatedAt = task.UpdatedAt, LastReminderAt = task.LastReminderAt, Status = task.Status.ToContractValue(), CompletedAt = task.CompletedAt, CancelledAt = task.CancelledAt, RecurringTaskId = task.RecurringTaskId };
    ...
    public async ValueTask<IReadOnlyList<TaskItem>> GetByRecurringTaskIdAsync(long recurringTaskId, CancellationToken cancellationToken) =>
        (await database.Tasks.AsNoTracking().Where(x => x.RecurringTaskId == recurringTaskId).OrderBy(x => x.Id).ToListAsync(cancellationToken)).Select(ToModel).ToList();
