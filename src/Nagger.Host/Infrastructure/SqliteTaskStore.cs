...
public sealed class SqliteTaskStore(NaggerDbContext database) : ITaskStore
{
    ...
    {
        var entity = new TaskEntity { Title = task.Title, DueAt = task.DueAt, ReminderPolicy = task.ReminderPolicy.ToContractValue(), CreatedAt = task.CreatedAt, UpdatedAt = task.UpdatedAt, LastReminderAt = task.LastReminderAt, Status = task.Status.ToContractValue(), CompletedAt = task.CompletedAt, CancelledAt = task.CancelledAt };
